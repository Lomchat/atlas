...
const suites = requested.length ? requested.map((name) => `verify-${name}.mjs`) : all;
for (const suite of suites) assert.ok(all.includes(suite), `Unknown suite: ${suite}`);
...
let passed = true;
// The same scenario runs in both languages at once: two software-WebGL browsers.
for (const suite of suites) {
  const pair = await Promise.all([run(suite, "en"), run(suite, "fr")]);
  passed = pair.every(Boolean) && passed;
}
writeFileSync("artifacts/test-matrix.json", JSON.stringify({ passed, results }, null, 2));
