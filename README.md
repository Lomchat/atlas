...
The default route opens the World Atlas: connected scientific views across the human body, a tree, a glass of water, a cloud, a rock and a mushroom. Local samples also explore soil, air, worked wood, glass, blood, bone and cellular material. The original molecular laboratory remains available at [`/lab`](https://atlas.chalco.website/lab).
...
A valid `?lang=en` or `?lang=fr` link takes priority over a saved preference, then the browser language; other browser languages default to English. World links include the selected `world` node ID and the `sites` address of selected local samples or surfaces. Laboratory links retain their scale and molecular `site=x,y,z` address. Shared links include the language.
...
- Approach a visible material region or anatomical part at the point that interests you. A leg vein opens a local venous sample while retaining that source vein's identity; bone, skeletal muscle and arteries have their own sample paths. The local-origin panel distinguishes a representative sample, a selected constituent and a point on a surface, and lets you return to that place. Hidden anatomical layers remain unpickable.
...
Dragging orbits the scene. With the world canvas focused, `+` or Enter follows the preferred child, `−` or Backspace goes outward, and Home returns to the landscape. Browser back/forward preserves the selected world path and local sample address. Saved discoveries retain that address too. The molecular laboratory has its own shortcuts below.
...
`sample-address.ts` records selected parent/child edges and local points separately from the composition tree. A material `sample` anchors a representative window; an `instance` anchors the chosen constituent; a `surface` selection changes camera focus without moving the object. `spatial-targets.ts` resolves material annotations to one appropriate direct child. `spatialOnly` nodes describe material already present, so they do not create extra floating objects or locator dots. The registry contains 8,238 addresses because shared lessons have separate ancestry in each context, not because it claims that many distinct concepts. `parts.json` maps every anatomical triangle to its actual source part; the compact table retains 1,670 identities within the existing 14 GLBs. Approximate region names support navigation, while exact FJ/FMA identifiers preserve provenance.
...
node scripts/spatial-models.mjs
...
`scripts/verify-anatomy.mjs` covers source identification, the four anatomical layers, visible-part picking, source-derived dimensions and anchors, anatomical return paths, language changes, phone controls and download/retry behavior. Its shared `assertAnatomyParts(locale)` guard reads the actual GLB buffers and checks every source range, identity, bound, hash and license, including unknown source names and approximate regions. It runs through the same bilingual matrix. `verify-spatial-world.mjs` covers local selection, source-preserving sample paths and address persistence. `scripts/mineral-models.mjs` supplies `assertMineralModels(locale)` to the world suite: it checks distinct grain/network and fibril/chain representations, quartz coordination and SI bond lengths, element-specific picking, bounded draw calls and resource disposal. Its standalone command runs the identical assertions in English and French. These are test scopes, not release validation results.

`node scripts/spatial-models.mjs` runs the same material-routing and geometry checks in English and French without opening a browser; `assertSpatialModels(locale)` also runs with `verify-spatial-world.mjs`. It checks direct-child destinations, selectable wall matrices versus fibers, source-linked plant/fungal explanations, explicit C/O identities, root-cell organelles and normalized/disposable models. The anatomy guard separately pins the reviewed geometry and provenance fingerprints so adding selection metadata cannot silently replace anatomical source meshes or licensing records.
...
Choosing a point does not turn a source surface into a microscope dataset. Tissue, cytoplasm, plasma and molecular views are representative educational samples at the selected location. Region labels such as “left leg” are approximate navigation hints; vessel type and source part remain separate, so an artery is not relabeled as a vein. An unnamed source part stays unnamed. Entering a surface also does not translate or enlarge the physical object to make the selected point its center.
