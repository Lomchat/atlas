...
- The two top buttons name their destinations. Repeated clicks immediately update the requested destination, including a reversal during a flight. All transitions interpolate distance logarithmically; all button flights last 300 ms with no animation queue. Reduced motion snaps to the destination.
- A permanent top reference compares the current scale with rice, a human hair, DNA, an atom, a nucleus or a proton. Its ruler tracks actual meters per CSS pixel at the camera target. Oversized comparisons are clipped and labeled rather than shrunk to fit.
...
The size-reference suite checks the actual rendered ruler against SI dimensions, fixed radii, nuclear containment, the atom-to-nucleus scale gap, hair reference selection, distant cell coordinates, continuous wheel zoom and responsive lesson layouts in both languages. Run it with `node scripts/test-matrix.mjs size-reference`.
